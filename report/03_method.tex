...
% TODO: Finish this. We need to explain distance driven as well as finish the explanation of the parameters in the function.
\noindent
Equation \ref{eq:fitness_distance_time} presents the fitness function that was used to evaluate genomes. This function aims towards letting the system train at one task at a time.

Note that if the distance driven $d$ is lower than the required distance to perform a lap $d_{max}$, or if the time $t$ that lap took is larger than the maximum time allowed $t_{max}$.

Note that the fitness is equal to the distance driven $d$, if $d$ is lower than $d_{max}$, the distance required to perform a lap or if the time $t$ that lap took is larger than the maximum time allowed $t_{max}$. This lets the system learn to complete a lap before it considers the time that lap took. This should reduce the complexity of performing a lap.

In order to properly balance the importance of the lap time and distance driven, the constant $\lambda$ was introduced. During the following experiments, $\lambda=5$ was found to be reasonable.
