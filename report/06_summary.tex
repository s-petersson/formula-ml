...
%Ellefsen (2015) show that modularity can be evolved in neural networks by neuroevolution techniequ. when t
%It has been shown that modularity can be evolved by neuroevolution techniques in order to adapt to new situations and avoiding to forget old behaviour \cite{ellefsen2015neural}.
...
% Interesting to swap machine learning algorithms.
% Recently deep reinforcement learning has shown great potential in solving problems with a continous action space [Source].
% Could potentially be used to completely alter the results achieved by using NEAT.

% Deep learning.

% One important problem is how the networks would be trained. Some suggestions are that the networks are trained in parallel, that they are part of the same genome but do not intersect. Another approach could be to train them in an alternating fashion.




% Modularity
% Deep reinforcement learning, Continuous DQN.
